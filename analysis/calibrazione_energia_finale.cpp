...
int main(int argc, char* argv[]) {
  ...
  for(int i=0;i<NCH;i++) ifs>>picco_FE_p[i]>>inc_picco_FE_p[i]>>picco_FE_d[i]>>inc_picco_FE_d[i];
  ...
  for(int channel=0; channel<NCH; channel++){
    ...
    inc_picco_test[channel] = ((1./k_cal[channel])*(1./k_cal[channel]))*(inc_picco_FE_d[channel])*(1./k_cal[channel])*(1./k_cal[channel])*(inc_picco_FE_d[channel]) + (picco_test[channel]/k_cal[channel])*(picco_test[channel]/k_cal[channel])*(inc_k_cal[channel]*inc_k_cal[channel]);
    
    k_test[channel] = MEDIA/picco_test[channel];
    inc_k_test[channel] = ((k_test[channel]/picco_test[channel])*(k_test[channel]/picco_test[channel]))*(inc_picco_test[channel]*inc_picco_test[channel]); //ERRORE QUADRATO PROPAGATO
  ...
  for(int channel=0; channel<NCH; channel++){
    ...
    inc_k_cal_normalizzato[channel]=inc_k_cal[channel]*inc_k_cal[channel]/(k_medio_cal*k_medio_cal) + (k_cal_normalizzato[channel]*k_cal_normalizzato[channel])/(k_medio_cal*k_medio_cal)*(inc_k_medio_cal*inc_k_medio_cal);
    ...
    inc_k_test_normalizzato[channel]=inc_k_test[channel]*inc_k_test[channel]/(k_medio_test*k_medio_test) + (k_test_normalizzato[channel]*k_test_normalizzato[channel])/(k_medio_test*k_medio_test)*(inc_k_medio_test*inc_k_medio_test);
