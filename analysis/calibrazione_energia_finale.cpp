...
int main(int argc, char* argv[]) {
	...
	for(int i=0;i<NCH;i++) efs >>picco_FE_p[i]>>inc_picco_FE_p[i];
	for(int i=0;i<NCH;i++) ofs >>picco_FE_d[i]>>inc_picco_FE_d[i];
	...
	for(int channel=0; channel<NCH; channel++){
    	...
    	inc_picco_test[channel] = (k_cal[channel]*k_cal[channel])*(inc_picco_FE_d[channel])*(inc_picco_FE_d[channel]) + (picco_FE_d[channel]*picco_FE_d[channel])*(inc_k_cal[channel]*inc_k_cal[channel]);
