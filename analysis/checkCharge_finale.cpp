...
int main(int argc, char* argv[]) {
  ...
  // file dove leggere il Tree
	...
	if(a==4){
		...
		TH1F* hist = new TH1F("hist", " ", 100, -20000, 10); // rapporto integrale/carica
    	...
    	TCanvas* c3 = new TCanvas("c3","Istogramma con tutte le cariche",600,800); // Nome, Titolo,x,y
  		...
  		while(i==0){
  			std::cout<<"Vuoi restringere il range? Premi 0 se si, 1 se no: "<<std::endl;
  			int b;
  			std::cin >> b;
  			if(b!=1 && b!=0){
  				std::cout<<"L'opzione selezionata non è valida, scegliere nuovamente tra le opzioni sopra."<<std::endl;
  			...
  			if(b==0){
  				...
  				std::cout<<"Scegliere il valore minimo: "<<std::endl;
  				std::cin >> min;
  				std::cout<<"Scegliere il valore massimo: "<<std::endl;
  				std::cin >> max;
  				std::cout<<"Scegliere il numero di bin: "<<std::endl;
  				std::cin >> bin;

				TH1F* hist_range = new TH1F("hist_range", " ", bin, min, max); // rapporto integrale/carica

				for (int j=0; j<NCH; j++) {
					for (int entry=0; entry<nEntries ; entry++) {
						tree->GetEntry(entry);
						hist_range->Fill(charge[j]);
					}
				}


    			TCanvas* c4 = new TCanvas("c3","Istogramma con tutte le cariche",600,800); // Nome, Titolo,x,y
	...
	//	PROGRAMMA NUMERO CINQUE
  	...
  	if(a==5){
		...
		TCanvas* c5 = new TCanvas("c5","Istogramma rapporti totale",600,800); // Nome, Titolo,x,y
 		c5->cd(); // Apre una sessione
 		c5->SetLogy();
 		totale->SetXTitle("I/C");
 		totale->SetYTitle("N eventi");
  		totale->Draw(); // Disegna l'istogramma
  		c5->SaveAs(Form("%s/hist_charge_totale.pdf", plotsDir.c_str()));
